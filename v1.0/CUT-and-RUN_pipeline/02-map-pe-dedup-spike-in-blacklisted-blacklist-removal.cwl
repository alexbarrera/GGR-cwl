...
class: Workflow
cwlVersion: v1.0
doc: 'CUT-and-RUN 02 mapping - reads: PE - removing duplicates - blacklist removal - spike in - spike in blacklist'
requirements:
...
steps:
   ...
   spike_in_blacklist:
     ...
     in:
       a: index_sorted_bams_spike_in/indexed_file
       b: spike_in_blacklist_bedfile
       output_basename_file: extract_basename_2/output_path
       v:
         valueFrom: ${return true}
     out:
     - file_wo_blacklist_regions
   sort_masked_bams_spike_in:
     run: ../map/samtools-sort.cwl
     scatter:
     - input_file
     in:
     ...
     out:
     - sorted_file
   index_masked_bams_spike_in:
     run: ../map/samtools-index.cwl
     scatter:
     - input_file
     in:
       input_file: sort_masked_bams_spike_in/sorted_file
     out:
     - indexed_file
   masked_file_basename_spike_in:
     run: ../utils/extract-basename.cwl
     scatter: input_file
     in:
       input_file: spike_in_blacklist/file_wo_blacklist_regions
